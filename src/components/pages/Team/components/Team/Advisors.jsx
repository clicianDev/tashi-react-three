    ...
    function Advisors() {
        return ( 
            <div className="grid sm:grid-cols-1 lg:grid-cols-3 xl:grid-cols-2 gap-10 w-full my-20 sm:px-5 md:px-20 lg:px-20 xl:px-32 m-auto">
        {Team.Advisors.map((item, index) => (
            <div
            key={index}
            className={`border border-tashi-tertiary rounded-2xl p-2 flex gap-5 h-max w-auto ${index === 4 ? '' : ''}`  }
            >
            <div className="border border-tashi-tertiary rounded-2xl flex gap-2 justify-center overflow-hidden w-max ">
                <img src={`./images/About/team/advisors/${item.imgPath}.png`} width={140} height={130}/>
            </div>
